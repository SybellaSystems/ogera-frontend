...
const AcademicRecords: React.FC = () => {
  ...
  const { data: myData, isLoading: myLoading } = useGetMyAcademicRecordsQuery(
    undefined,
    {
      skip: isSuperAdmin,
    },
  ...
  const { data: allData, isLoading: allLoading } =
    useGetAllAcademicRecordsQuery(undefined, {
      skip: !isSuperAdmin,
    });
  const [addAcademicRecord, { isLoading: submitting }] =
    useAddAcademicRecordMutation();
  const [deleteAcademicRecord, { isLoading: deleting }] =
    useDeleteAcademicRecordMutation();
  ...
  const [recordToDelete, setRecordToDelete] =
    React.useState<AcademicRecord | null>(null);

  const records = (isSuperAdmin ? allData?.data : myData?.data) || [];

  const averagePercentage =
    records.length > 0
      ? records.reduce((sum, item) => sum + Number(item.percentage || 0), 0) /
        records.length
      : 0;
  ...
  const getDisplayProfile = (record: any): "schooling" | "college" => {
    if (
      record.academic_profile === "schooling" ||
      record.academic_profile === "college"
    ) {
      return record.academic_profile;
    }
    if (record.academic_profile === "10" || record.academic_profile === "12")
      return "schooling";
  ...
  const getDisplayDetails = (record: any): string => {
    ...
    if (profile === "schooling") {
      const className =
        record.class_name ||
        (record.academic_profile === "10" || record.academic_profile === "12"
          ? record.academic_profile
          : "-");
    ...
    const degree =
      record.degree ||
      (record.academic_profile === "graduation" ? "Graduation" : "-");
  ...
  const handleViewCertificate = async (recordId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/academic-records/${recordId}/certificate`,
        {
          method: "GET",
          credentials: "include",
          headers: accessToken
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {!isSuperAdmin && (
        <form
          ...
          className="bg-white border border-gray-200 rounded-2xl p-5 grid grid-cols-1 md:grid-cols-2 gap-4 shadow-sm"
        >
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((s) => ({
                  ...s,
                  academic_profile: e.target.value as "schooling" | "college",
                }))
              }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // Allow only numbers 1-12 while typing
                    if (
                      value === "" ||
                      (/^\d+$/.test(value) &&
                        Number(value) >= 1 &&
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // Allow only letters, spaces, commas, periods, apostrophes, and parentheses
                    if (/^[A-Za-z\s,.'()]*$/.test(value)) {
                      setForm((s) => ({ ...s, board: value }));
                    }
                  }}
                  className="w-full rounded-lg border border-gray-300 px-3 py-2"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // Allow only letters and dots
                    if (/^[A-Za-z.]*$/.test(value)) {
                      setForm((s) => ({ ...s, degree: value }));
                    }
                  }}
                  className="w-full rounded-lg border border-gray-300 px-3 py-2"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // Allow only alphabets, spaces, comma, dot, parentheses, and apostrophe
                    if (/^[A-Za-z\s,.'()]*$/.test(value)) {
                      setForm((s) => ({ ...s, university: value }));
                    }
                  }}
                  className="w-full rounded-lg border border-gray-300 px-3 py-2"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                // Allow typing A-E followed by optional 1 or 2
                if (/^[A-E]?[1-2]?$/.test(value)) {
                  setForm((s) => ({ ...s, grade: value }));
                }
              }}
              className="w-full rounded-lg border border-gray-300 px-3 py-2"
              placeholder="A1, A2, B1, B2, etc."
            />
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-[#7F56D9]/5 to-transparent">
          ...
          <div className="flex flex-wrap gap-3 text-sm">
            <span className="inline-flex items-center rounded-lg bg-gray-50 border border-gray-200 px-3 py-1.5 text-gray-700">
              Avg %:{" "}
              <strong className="ml-1 text-[#7F56D9]">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          type="button"
                          onClick={() =>
                            handleViewCertificate(record.record_id)
                          }
      ...
      <ConfirmDeleteModal
        ...
        itemName={
          recordToDelete ? getDisplayDetails(recordToDelete) : undefined
        }
