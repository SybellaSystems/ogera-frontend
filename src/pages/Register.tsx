...
const Register = () => {
  ...
  useEffect(() => {
    ...
    if (data && isSuccess) {
      ...
      // Persist verification context so /auth/verification can work without any query params.
      ...
      // Optional development context if backend returns it.
      const phoneNumber = (data as any)?.data?.phoneNumber;
      if (phoneNumber) {
        localStorage.setItem("pendingVerificationPhoneNumber", phoneNumber);
      }

      // Also reset form after capturing the email.
      resetForm();

      // Show verification message and navigate after a delay
      setTimeout(() => {
        navigate("/auth/login", {
          state: { showVerificationMessage: true, showVerifyAccountButton: true, email: registeredEmail },
        });
      }, 2000);
    }
